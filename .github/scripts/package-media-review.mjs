...
if (!candidatePath || !outputDirectory) throw new Error("Expected candidate path and output directory.");
const MAX_REDIRECTS = 3;
const MAX_BYTES = 12 * 1024 * 1024;
const TYPES = new Map([["image/avif","avif"],["image/jpeg","jpg"],["image/png","png"],["image/webp","webp"]]);
...
async function fetchRaster(initialUrl) {
  ...
  for (let index = 0; index <= MAX_REDIRECTS; index += 1) {
    ...
    const response = await fetch(current, {redirect:"manual",signal:AbortSignal.timeout(10000),headers:{accept:"image/avif,image/webp,image/png,image/jpeg;q=0.9,*/*;q=0.1","user-agent":"Tessli reviewed-media evidence/1.0"}});
    if ([301,302,303,307,308].includes(response.status)) {
      const location = response.headers.get("location");
      if (!location || index === MAX_REDIRECTS) throw new Error(`Unsafe or excessive redirect from ${current}`);
    ...
    if (!bytes.length || bytes.length>MAX_BYTES) throw new Error("Raster is empty or exceeds byte limit.");
...
for (const resource of source.resources??[]) for (const kind of ["preview","favicon"]) {
  ...
  try {
    const result=await fetchRaster(media.url); const fileName=`${resource.resourceId}-${kind}.${TYPES.get(result.contentType)}`;
    await fs.writeFile(path.join(outputDirectory,fileName),result.bytes);
    Object.assign(entry,{status:"downloaded",fileName,finalUrl:result.finalUrl,redirects:result.redirects,contentType:result.contentType,bytes:result.bytes.length,sha256:crypto.createHash("sha256").update(result.bytes).digest("hex")});
