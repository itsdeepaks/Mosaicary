...
ROOT=pathlib.Path('.')
D=json.loads((ROOT/'.github/data/media-batch-12-decisions.json').read_text())
...
sel=read(ROOT/f'artifacts/media-batch-{B}/selection.json'); cand=read(ROOT/f'artifacts/media-batch-{B}/candidates.json')
...
media_path=ROOT/'lib_data/resource-media.json'; media=read(media_path); old_approved=len(media['resources']); existing={x['resourceId'] for x in media['resources']}
...
media['resources'].sort(key=lambda x:order[x['resourceId']]); write(media_path,media); new_approved=old_approved+len(A)
cov_path=ROOT/'lib_data/resource-media-coverage.json'; cov=read(cov_path); by={x['resourceId']:x for x in cov['resources']}; old_pending=sum(x['disposition']=='pending' for x in cov['resources']); old_terminal=len(cov['resources'])-old_approved-old_pending
for rid in IDS:
 x=by[rid]
...
write(cov_path,cov); new_pending=old_pending-len(IDS); new_terminal=old_terminal+len(T)
