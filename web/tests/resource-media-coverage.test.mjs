...
test("coverage manifest represents all catalogue resources deterministically", async () => {
  ...
  assert.equal(repositoryCoverage.summary.approvedMedia, 139);
  assert.equal(repositoryCoverage.summary.pending, 67);
  assert.equal(repositoryCoverage.summary.terminalWithoutMedia, 89);
  assert.equal(
